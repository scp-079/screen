...
REGEX 能方便的控制 SCP-079 系列的机器人所使用的正则规则，若无 REGEX 配合，NOSPAM、WATCH、CLEAN 将无法正常工作。
...
3. 本文中假设 SCP-079 旗下所有机器人的所有功能均被开启，实际上本文所述的所有封禁、删除均受`白名单`、`操作降级`和`群组对机器人的配置`共同调控。
...
参与管理 REGEX 机器人的成员，应在 SCP-079-REGEX 专用群组中进行操作。并在 SCP-079-TEST 中对所添加规则进行测试。
...
- NOSPAM：将使用该类规则分析用户头像 OCR 结果，触发该类规则集将导致全局封禁触发者，并加入全局黑名单。

### 敏感检测

规则集代号：bad

- NOSPAM：更新用户评分，自动举报至 WARN，令其提交至群组管理员处理
...
- NOSPAM：检测用户昵称和简介，全局封禁触发者，并加入全局黑名单。
...
- NOSPAM：删除触发的消息并更新触发用户评分

### 文件名称

规则集代号：fil

- NOSPAM：封禁触发用户并更新用户评分
...
- CLEAN：用于判断 Telegram 链接

### TG 代理

规则集代号：tgp

- CLEAN：用于判断与 Telegram 有关的代理
...
您可使用 `/ls 规则集代号` 显示该规则集中的全部内容，该列表默认按照平均触发次数的降序排列，使用 `/ls 规则集代号 asc` 将按照平均出发次数的升序显示该规则集。
...
如需将相同的正则表达式添加至多个规则集中，可使用 `/same 规则集代号1 规则集代号2 规则集代号3 规则集代号4` 回复某 `/add` 命令完成。

在同规则集中添加与已有正则表达式近似的正则表达式时将触发替换询问。
...
如需将相同的正则表达式从多个规则集中删除，可使用 `/same 规则集代号1 规则集代号2 规则集代号3 规则集代号4` 回复某 `/remove` 命令完成。
