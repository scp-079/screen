...
本节介绍使用 SCP-079-REGEX 管理本项目机器人使用的正则表达式的部分理念，该节内容仅供参考。
...
`ad\* 规则集`间，`ad\*规则集`与`con规则集`、`iml规则集`、`pho规则集间`，以及`ad规则集`与`ad\*规则集`与`con规则集`、`iml规则集`、`pho规则集间`，均应当避免重叠，这样做能减少误判。
...
SCP-079-NOSPAM 将提取正则表示式中名为`con`的捕获组，并去除其中可能存在的空白字符。
示例：
在 SCP-079-REGEX 输入
...
**注1：**以上内容仅为示例，实际环境中检测 `QQ 号` 的正则表达式远比该示例复杂。
...
将导致 SCP-079-NOSPAN 封禁该用户，并记录该联系方式 `123456789`，记录结果将于证据存放频道公示。

SCP-079-MANAGE 群组成员可使用 `/remove_contact 内容`移除已收录的联系方式（例如：`/remove_contact 123456789`）。收录的联系方式也将于一定时间后自动清空。


**注2：**未实际导致用户遭受封禁则不进行联系方式收录。

**注3：**NOSPAM将只收集`con规则集`、`iml规则集`、`pho规则集间`中名为`con`的捕获组，NOSPAM不收录其他规则集中的`con`的捕获组，也不收录`con规则集`、`iml规则集`、`pho规则集间`中的其他捕获组。

**注4：**NOSPAM不收录少于6字符的联系方式\[暂未实现\]，防止内容过短导致的误判。
