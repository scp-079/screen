...
![多用户入群时的常规提示](/images/captcha-zh/multi.png)

![待验证用户点击「我需要验证吗」按钮后，收到的提示](/images/captcha-zh/check.png)
...
如待验证用户在有限时间内回答错误，机器人将封禁该用户 n 小时（其中 n = 2^m ，m 为验证失败次数）。同时，用户会收到机器人的弹窗提示，告知用户回答错误，验证失败，重试需要等待 n 小时。用户可在 n 小时后再次入群触发验证。

![用户回答错误收到的弹窗提示](/images/captcha-zh/qns-wrong.png)
...
因自定义问题超时导致的验证失败，机器人将封禁相应用户 24 * n 小时（其中 n = 2^m ，m 为验证失败次数）。用户可在 24 * n 小时后再次入群触发验证。
...
![以 TXT 文件形式显示所有问题](/images/captcha-zh/qns-show-file.png)
